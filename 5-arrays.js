...
//EXTRA ARRAY CONCEPTS
//Shift and Unshift methods
//Shift method will add the element to the start of the array and shift others to the right
//Unshift method will remove the element from the beginning and shifts others to left
let newArr19 = [1, 2, 3];
newArr19.shift(0); //This will add 0 at beginning
console.log(newArr19); //[0,1,2,3]
newArr19.unshift(); //This will remove 0 from the beginning
console.log(newArr19); //[1,2,3]

//COMMONLY USED ARRAY METHODS
let arr20 = new Array(5).fill(1).map((item, index) => index + 1); //[1,2,3,4,5]
arr20.includes(1); //true
arr20.includes(0); //false
arr20.join("-"); //1-2-3-4-5
arr20.join(""); //12345
arr20.length; //5
let arr21 = arr20.concat([6, 7]); //[1,2,3,4,5,6,7] - NOTE that concat will not add to existing array
arr21 = arr21.concat(8); //[1,2,3,4,5,6,7,8] - NOTE you need to store new array in variable after concat
console.log(arr20); //[1,2,3,4,5]
arr20.push(6); //[1,2,3,4,5,6]
arr20.pop(); //[1,2,3,4,5]
arr20.indexOf(1); //0 - Index of where the element is present
arr20.indexOf(0); //-1 - When the element is not present
let arr22 = arr20.slice(0, 2); //[1,2]
Array.isArray(arr22); //true - To check if a variable is holding an array
Array.isArray({}); //false
let sentence = "This is a simple sentence";
let words = sentence.split(" "); //["This","is","a","simple","sentence"]
let sentenceBack = words.join(" ");
console.log(words.toString()); //This,is,a,simple,sentence
